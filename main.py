...
# dont need ? plt.plot(Doge.index, Doge["Price"], color='red')

Bit = pd.read_csv("Bitcoin Historical Data.csv")
# dont need plt.scatter(x=Bit["Date"], y=Bit["Price"])
...
fig, ax = plt.subplots()
ax.plot(Ggl_AMD_Compare["Open_Ggl"], color='red')
ax.set_ylabel("Google stock Price", color='red')
ax2 = ax.twinx()
ax2.plot(Ggl_AMD_Compare["Open_AMD"], color='blue',)
ax2.set_ylabel("AMD stock price", color='blue')
ax.set_xlabel("Google Vs AMD stock price growth")
